...
import { updateItemInCart } from "../services/cartServices";
import { deleteItemInCart } from "../services/cartServices";
...
router.delete(
  ...
  async (req: ExtendRequest, res) => {
    try {
      ...
      res.status(response.statusCode).json(response.data || { items: [], totalAmount: 0 });
    } catch (err) {
      console.error(err);
